...
| [`tests/run-tests.sh`](tests/run-tests.sh) | the calibration harness — crash recovery, the emitter mutex, `verify`, rulings, invariants, token capture, pane reuse, agent memory, continuity, agent serialisation, the dispatch line reaching a pane and a result landing mid-tick. `43 passed, 0 failed` |
